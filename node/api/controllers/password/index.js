...
const FILE_BASE = process.env.THIS_BASE_PATH + '/data/password/';
...
exports.handler = async (event, context, callback) => {
	...
	if( event.path == '/pwd-allpasswd' ){
		var list = [];
		pwd.list.forEach(item => list.push({ name: item.name, userid: item.userid, password: item.password } ));
		return new Response({ result: list });
	}else
